package com.transsion.imagepicker.view;
...
public class CropVideoView extends AppCompatImageView {
    ...
    private final int TRANSPARENT = 0x0000;   //透明
    private int mMaskColor = 0x80101112;   //暗色
    ...
    public CropVideoView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mShadowPaint.setColor(TRANSPARENT);
    ...
    private void initImage() {
        ...
        mFocusRect.left = mFocusMidPoint.x - mFocusWidth / 2f;
        mFocusRect.right = mFocusMidPoint.x + mFocusWidth / 2f;
        mFocusRect.top = mFocusMidPoint.y - mFocusHeight / 2f;
        mFocusRect.bottom = mFocusMidPoint.y + mFocusHeight / 2f;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (Style.RECTANGLE == mStyle) {
            ...
            mClipPath.addRect(mFocusRect, Path.Direction.CCW);
            canvas.save();
            ...
            float radius = Math.min((mFocusRect.right - mFocusRect.left) / 2f, (mFocusRect.bottom - mFocusRect.top) / 2f);
            mFocusPath.addCircle(mFocusMidPoint.x, mFocusMidPoint.y, radius, Path.Direction.CCW);
            mClipPath.addCircle(mFocusMidPoint.x, mFocusMidPoint.y, radius, Path.Direction.CCW);
            canvas.save();
            canvas.clipRect(0, 0, getWidth(), getHeight());
            canvas.clipPath(mFocusPath, Region.Op.DIFFERENCE);
            canvas.drawColor(mMaskColor);
     ...
     * @param canvas
     */
    private void drawOuterShadow(Canvas canvas) {
        ...
        mShadowPaint.setXfermode(mXFerMode);
    ...
    private Bitmap makeCropBitmap(Bitmap bitmap, RectF focusRect, RectF imageMatrixRect, int expectWidth, int exceptHeight, boolean isSaveRectangle) {
        ...
        try {
            ...
            if (expectWidth != width || exceptHeight != height) {
                ...
                if (mStyle == CropVideoView.Style.CIRCLE && !isSaveRectangle) {
                    //如果是圆形，就将图片裁剪成圆的
