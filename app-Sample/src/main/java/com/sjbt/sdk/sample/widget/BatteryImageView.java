...
public class BatteryImageView extends AppCompatImageView {
    ...
    private void init() {
        mUnknownDrawable = ResourcesCompat.getDrawable(getResources(), R.mipmap.ic_battery_unknown, getContext().getTheme());
        mChargingDrawable = ResourcesCompat.getDrawable(getResources(), R.mipmap.ic_battery_charging, getContext().getTheme());
    ...
    private Drawable createPercentageDrawable(boolean warn, int percentage) {
        Drawable bgDrawable = ResourcesCompat.getDrawable(getResources(), R.mipmap.ic_battery_zero, getContext().getTheme());
        ClipDrawable percentageDrawable;
        if (warn) {
            percentageDrawable = new ClipDrawable(ResourcesCompat.getDrawable(getResources(), R.mipmap.ic_battery_low, getContext().getTheme()), Gravity.START, ClipDrawable.HORIZONTAL);
        } else {
            percentageDrawable = new ClipDrawable(ResourcesCompat.getDrawable(getResources(), R.mipmap.ic_battery_full, getContext().getTheme()), Gravity.START, ClipDrawable.HORIZONTAL);
        }
    ...
    private Drawable createWarnDrawable(int percentage) {
        ...
        animationDrawable.addFrame(ResourcesCompat.getDrawable(getResources(), R.mipmap.ic_battery_zero, getContext().getTheme()), 500);
