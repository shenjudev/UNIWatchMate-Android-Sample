 ...
 *
 * 【重要说明】
 * 这个 Demo 完全独立于原始的 SJ SDK，不依赖 MsgBean 等内部类。
 * 使用 UNIWatchMate.getDataOutputStream() 获取蓝牙输出流，无需反射。
 *
 * 用户只需要：
...
class OtaDemoActivity : AppCompatActivity() {
    ...
    companion object {
        private const val TAG = "OtaDemoActivity"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        initOtaSdk()
        setupClickListeners()
        UNIWatchMate.observeBluetoothReceiveData.subscribe {
            onBluetoothDataReceived(it)
        }
    ...
    private fun initOtaSdk() {
        ...
        appendLog("注意：需要实现实际的蓝牙通信逻辑")
        appendLog("消息接收：需要在 SJUniWatch 的消息处理中调用 onBluetoothDataReceived()")
    }
     ...
     *
     * 【简化版本】
     * 使用 UNIWatchMate.getDataOutputStream() 直接获取蓝牙输出流，
     * 无需使用反射等复杂方式。
     */
        ...
        return object : IBluetoothCommunicator {
            override fun sendMessage(data: ByteArray) {
                try {
                    ...
                    // 使用 UNIWatchMate 提供的接口获取输出流
                    val outputStream = UNIWatchMate.getDataOutputStream()
    ...
    private fun startTransfer() {
        ...
        // 检查蓝牙连接状态
        if (UNIWatchMate.getDataOutputStream() == null) {
            Toast.makeText(
                this,
                "蓝牙未连接，请确保设备已连接",
                Toast.LENGTH_LONG
